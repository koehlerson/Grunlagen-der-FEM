...
c         Default user constitutive equation number
          ...
          call uconst(text(2),ev, d(uprm+1),n1,n3, umat)
...
c     Save types
      ...
      if(ietype.eq.1 .or. ietype.eq.5) then
        ...
        if(etype.eq.3 .and. sflag) nh1 = nh1 + 60 ! linear element
...
c     Set modulus
      ...
      subroutine epps2d(d,eps,epsp,alp,epn, sig,dd,dr)
...
c     Compute scale factor and compute elastic-plastic tangent
      ...
      if (yldtr.gt.rad2 .and.state) then
...
c              H_visc(x) = [ 1 - exp(-x)]/x
     ...
     &               - 0.25d0*x*(1.d0 - 0.2d0*x)))
      ...
      subroutine mises(d,eps,epsp,epp, sig,dd,dr)
...
c     Check state for iterations
      ...
      if(rnmax.ne.0.0d0) then
        state = .true.           ! Not first iteration
      else
        state = .false.          ! First iteration in step
      endif
...
c     Check yield
      ...
      if(xin .gt. (Rn+Rb) .and. state) then
...
c     Compute tangent moduli
      ...
      subroutine modl1d(d,ta,eps,hn,h1,nh,ii,dd,sig,isw)
...
c       P l a s t i c i t y
        ...
        if(plasfl) then
          ...
          call plas1d(d,ta,eps,h1, sig,dd)
...
c     Rayleigh stress
      ...
      subroutine modlsd(d,ta,eps,h1,h2,nh,dd,sig,isw)
...
c           Plane stress plasticity
            ...
            if (stype.eq.1 .or.stype.eq.4) then
              ...
              call epps2d(d,eps,h2,h2(4),h2(7), sig,dd,dd(1,1,5))
...
c           Plane strain, axisymmetric or 3D plasticity
              ...
              call mises(d,eps,h2(3),h2, sig,dd,dd(1,1,5))
...
c       Compute trace to pass to user module
     ...
     &              sig,dd,isw)
      ...
      subroutine plas1d(d,ta, eps,h1, sig,dd)
...
c     Model 1.) h(J) = (J - 1)
      ...
      subroutine modlfd(d,f,finv,df,detf,ta,hn,hn1,nh,dd,sig,bb,
     &                  xlamd,ha, bbar, isw)
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      logical   bbar, plasfl
      integer   nh,isw, i,j, imat, ntm, uprm,umat
      real*8    ta,xlamd,ha
