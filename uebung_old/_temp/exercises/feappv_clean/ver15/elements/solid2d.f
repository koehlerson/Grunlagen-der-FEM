...
c....  Copyright (c) 1984-2004: Robert L. Taylor
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      integer   ndf,ndm,nst,isw
      integer   i,i1, j,jj,j1, l,lint, nn,nhv
...
c       Compute Cauchy stresses and spatial tangent tensor
     ...
     &             hr(nn+nh1),hr(nn+nh2),nhv,ds,sigv,bb,
     &             xlamd,ha,.false.,isw)
...
c       Compute Cauchy stresses and spatial tangent tensor at t-n+1
        ...
        call modlfd(d,f(1,1,l),finv(1,1,l),df(1,1,l),detf(1,l),ta,
     &             hr(nn+nh1),hr(nn+nh2),nhv,ds,sigl(1,l),bb,
     &             xlamd,ha,.false.,isw)
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      integer   ndf,ndm,nst, isw
      integer   i,i1, j,jj,j1, l,lint, nhi,nhv,nn
...
c         Compute Cauchy stresses and spatial tangent tensor at t-n+1
     ...
     &                  hr(nn+nh1),hr(nn+nh2),nhv,ad(1,1,1,l),sigl(1,l),
...
c           Compute Cauchy stresses and spatial tangent tensor at t-n+1
            ...
            call modlfd(d,fi(1,1,l),finv(1,l),df(1,l),theta(1,l),ta,
     &                  hr(nn+nh1),hr(nn+nh2),nhv,ad,sigl(1,l),
     &                  bei,hr(nh2),hr(nh2+1),.true.,isw)
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      integer   ndm
      real*8    xsj, sh,th,xs0,xt0,ys0,yt0
...
c     Set up interpolations
      ...
      sh  =  0.5d0*sg(1)
      th  =  0.5d0*sg(2)
      xs0 = -xl(1,1) + xl(1,2) + xl(1,3) - xl(1,4)
      xt0 = -xl(1,1) - xl(1,2) + xl(1,3) + xl(1,4)
      ys0 = -xl(2,1) + xl(2,2) + xl(2,3) - xl(2,4)
      yt0 = -xl(2,1) - xl(2,2) + xl(2,3) + xl(2,4)
...
c     Plane and axisymmetric linear elastic element routine
      ...
      logical   quad
      integer   ndf,ndm,nst,isw
      integer   j,k,l,ii,j1,k1,lint,nhv,nn
...
c         Compute stresses and strains
     ...
     &                xx,yy,ta,eps)
          call modlsd(d,ta,eps,hr(nh1+nn),hr(nh2+nn),nhv,
     &                dd,sig(1,l),isw)
...
c     Plane/axisymmetric linear element routine - B-bar formulation
      ...
      integer   i,j,ii,i1,jj,j1,k,l,lint,nhv,nn
...
c         Compute Cauchy stresses and spatial tangent tensor at t-n+1
          ...
          call modlsd(d,ta,eps,hr(nn+nh1),hr(nn+nh2),nhv,
     &                aa,sig,isw)
...
c      hr(0)            Enhanced displacement      ui        4*2
c      hr(4)            Increment to enhanced ui   -         4
c      hr(8)            Enhanced residual          bb        4
c      hr(12)           Coupling matrix            gg       32
c      hr(44)           Enhanced stiffness         hh       16
c      hr(60)           Stress history at point-1   -      nhv
c      hr(60+  nhv)     Stress history at point-2   -      nhv
c      hr(60+2*nhv)     Stress history at point-3   -      nhv
c      hr(60+3*nhv)     Stress history at point-4   -      nhv

c      Total number of words / element is 60 + 4*nhv
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8    dv,rn,zn,sn,rr,zz
      real*8    a11,a21,a41,a12,a22,a42,b1,b2,ta
      integer   i,j,l,lint,nhv,nu1,nu2,nd2,nn,i1,j1,i2,j2

      integer   ix(*)

      real*8    d(*),xl(ndm,*),ul(ndf,*),tl(*),s(nst,*),p(nst),xsj(9)
      real*8    shp(3,9,9),sg(3,16),sig(9,9),eps(9,3),du(8)
      real*8    gg(4,8),hh(4,4),bb(4),hg(4,8),hb(4),dui(4),dd(6,6,5)
      real*8    ss(8,8)
...
c     Recover enhanced modes (saved in last iteration)
      ...
      call pmove(hr(nh2+ 8),bb, 4)
      call pmove(hr(nh2+12),hg,32)
      call pmove(hr(nh2+44),hh,16)

c     Move incremental displacements to a local array

      call pmove(ul(1,1+2*nen),du,8)
      nu1 = nh1 - 1
      nu2 = nh2 - 1
      nd2 = nh2 + 3
      do i = 1,4
        dui(i) = bb(i)
        do j = 1,8
          dui(i) = dui(i) - hg(i,j)*du(j)
        end do
        hr(nd2+i) = hr(nd2+i) + dui(i)
        hr(nu2+i) = hr(nu1+i) + hr(nd2+i)
c       hr(nu2+i) = hr(nu1+i) + dui(i)
        ui(i,1)   = hr(nu2+i)
        ui(i,2)   = ui(i,1) - hr(nu1+i)
      end do
...
c       Set initial counter for history terms in stress/strain
        ...
        nn = 60
        do l = 1,lint
          call shp2d(sg(1,l),xl,shp,xsj(1),ndm,nel,ix,.false.)

c         Compute enhanced strain shape functions

          call shpi2d(sg(1,l),xsj(1),xl,ndm)
          dv = xsj(1)*sg(3,l)

c         Compute stress and strain at point

          call strn2d(d,xl,ul,tl,shp,ndf,ndm,nel,rr,zz,ta,eps)

          call modlsd(d,ta,eps,hr(nn+nh1),hr(nn+nh2),nhv,dd,sig,isw)
...
c           Compute gravity and stress divergence loads

            p(j1  ) = p(j1  ) +b1*sn - rn*sig(1,1) - sig(4,1)*zn
            p(j1+1) = p(j1+1) +b2*sn - zn*sig(2,1) - sig(4,1)*rn
            j1 = j1 + ndf
          end do
...
c           Compute enhanced stress divergence loads
            ...
            bb(2*j-1) = bb(2*j-1) - sig(1,1)*rn - sig(4,1)*zn
            bb(2*j  ) = bb(2*j  ) - sig(2,1)*zn - sig(4,1)*rn
          end do
...
c             Compute d * b matrix = a
              ...
              rn  = shp(1,j,1)*dv
              zn  = shp(2,j,1)*dv
              a11 = dd(1,1,1)*rn + dd(1,4,1)*zn
              a21 = dd(2,1,1)*rn + dd(2,4,1)*zn
              a41 = dd(4,1,1)*rn + dd(4,4,1)*zn
              a12 = dd(1,2,1)*zn + dd(1,4,1)*rn
              a22 = dd(2,2,1)*zn + dd(2,4,1)*rn
              a42 = dd(4,2,1)*zn + dd(4,4,1)*rn
              i1 = 1
              do i = 1,j
                rn = shp(1,i,1)
                zn = shp(2,i,1)
                s(i1  ,j1  ) = s(i1  ,j1  ) + rn*a11 + zn*a41
                s(i1  ,j1+1) = s(i1  ,j1+1) + rn*a12 + zn*a42
                s(i1+1,j1+1) = s(i1+1,j1+1) + zn*a22 + rn*a42
                s(i1+1,j1  ) = s(i1+1,j1  ) + zn*a21 + rn*a41
                i1 = i1 + ndf
              end do
              do i = 1,2
                rn = shpi(1,i)
                zn = shpi(2,i)
                gg(2*i-1,2*j-1) = gg(2*i-1,2*j-1) + rn*a11 + zn*a41
                gg(2*i-1,2*j  ) = gg(2*i-1,2*j  ) + rn*a12 + zn*a42
                gg(2*i  ,2*j  ) = gg(2*i  ,2*j  ) + zn*a22 + rn*a42
                gg(2*i  ,2*j-1) = gg(2*i  ,2*j-1) + zn*a21 + rn*a41
              end do
              j1 = j1 + ndf
            end do
            do j = 1,2
...
c             Compute d * b matrix = a
              ...
              do i = 1,2
                rn = shpi(1,i)
                zn = shpi(2,i)
                hh(2*i-1,2*j-1) = hh(2*i-1,2*j-1) + rn*a11 + zn*a41
                hh(2*i-1,2*j  ) = hh(2*i-1,2*j  ) + rn*a12 + zn*a42
                hh(2*i  ,2*j  ) = hh(2*i  ,2*j  ) + zn*a22 + rn*a42
                hh(2*i  ,2*j-1) = hh(2*i  ,2*j-1) + zn*a21 + rn*a41
              end do
            end do
          endif
          nn = nn + nhv
...
c       Eliminate enhanced modes
        ...
        if(isw.eq.3) then
          ...
          do i = 1,4
            do j = 1,8
              hg(i,j) = dot(hh(1,i),gg(1,j),4)
            end do
          end do
          do j = 1,8
            do i = 1,8
              ss(i,j) = dot(gg(1,i),hg(1,j),4)
            end do
          end do
...
c         Construct symmetric part
          ...
          do j = 1,4
            ...
            i2 = 0
            do i = 1,4
              s(i1+1,j1+1) = s(i1+1,j1+1) - ss(i2+1,j2+1)
              s(i1+1,j1+2) = s(i1+1,j1+2) - ss(i2+1,j2+2)
              s(i1+2,j1+1) = s(i1+2,j1+1) - ss(i2+2,j2+1)
              s(i1+2,j1+2) = s(i1+2,j1+2) - ss(i2+2,j2+2)
              i1           = i1 + ndf
              i2           = i2 + 2
            end do ! i
            j1 = j1 + ndf
...
c         Save enhanced strain arrays for later

          call pmove(hg,hr(nh2+12),32)
          call pmove(hh,hr(nh2+44),16)
        endif

c       Compute and save residual

        do i = 1,4
          hb(i)   = dot(hh(1,i),bb,4)
        end do
        call pmove(hb,hr(nh2+ 8), 4)

        j1 = 0
        j2 = 0
        do j = 1,4
          p(j1+1)   = p(j1+1)   - dot(hg(1,j2+1),bb(1),4)
          p(j1+2)   = p(j1+2)   - dot(hg(1,j2+2),bb(1),4)
          j1        = j1 + ndf
          j2        = j2 + 2
        end do
...
c         Compute principal stress values
          ...
          if(isw.eq.4) then
            ...
            write(iow,2002) n,rr,(sig(i,l),i=1,6),
     &                      ma,zz,(eps(i,1),i=1,4),sig(7,l)
            if(ior.lt.0) then
              write(*,2002) n,rr,(sig(i,l),i=1,6),
     &                      ma,zz,(eps(i,1),i=1,4),sig(7,l)
            endif
          endif
          nn = nn + nhv
...
2001  format(a1,20a4//5x,'Element Stresses'//' Elmt 1-coord',
     ...
     &    3x,'1-stres    2-stress'/' matl 1-coord  11-strain',
     &    2x,'22-strain  33-strain  12-strain      angle'/
     &    39(' -'))
2002  format(i4,0p1f9.3,1p6e11.3/i4,0p1f9.3,1p4e11.3,0p1f11.2/)
