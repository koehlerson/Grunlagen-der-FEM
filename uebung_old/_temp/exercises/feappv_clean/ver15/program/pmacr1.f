...
c     [stre,node,k1,k2,k3]    - output nodal stresses k1 to k3 inc k3
c                               (lumped projection)
...
c       Projections to nodes
     ...
     &                1,numel,1)
          call pltstr(hr(np(58)),hr(np(57)+numnp),hr(nty),numnp,ndm)
        endif

c       Output sets

        call prtstr(hr(np(43)),hr(np(57)+numnp),hr(nty),
...
c       Compute flexible FE contributions to arrays
...
c           Line search - linear search along solution direction
     ...
     &                      hr(np(26)),ct(3,l),hr(np(82)),neq,step)
...
c       No active equation update case
...
c       Initialize for consistent mass
        ...
        if(fl(1).and.fl(9)) then
          setvar = palloc(1,'TANGS',mr(np(21)+neq-1)+neq,2)
          na = np(1)
...
c       Initialize for lumped mass
        ...
        elseif(fl(9)) then
          if(fl(2)) then
            ...
            setvar = palloc(82,'TEMP2',neq, 2)
            ml1   = np(82)
            imtyp = 1
            call formfe(np(40),ml1,ml1,ml1,fa,tr,fa,5,1,numel,1)
          endif

          if(cknon0(hr(ml1),neq)) then
            setvar = palloc(81,'TEMP1',neq, 2)
            call piacel(hr(ml1),hr(np(26)),hr(np(42)+nneq),neq)
            call pexpd(hr(np(42)+nneq),hr(np(81)),mr(np(31)),
     &                 neq,nneq)
            setvar = palloc(81,'TEMP1',0, 2)
          else
            if(ior.lt.0) then
          ...
          if(np(82).gt.0) setvar = palloc(82,'TEMP2',0, 2)
...
8     setvar = palloc(17,'DAMPS',mr(np(21)+neq-1)+neq,2)
      nc = np(17)
...
c --- [augm,,value] perform nested update for augmented lagrangian
...
c     [direct,sparse] ! N.B. Users must furnish their own
...
c     Profile solution
        ...
        if(np(1).eq.0 .and. k1.gt.maxm-mmax) then
          write(iow,3007) 'Symmetric',k1+mmax,maxm
          if(ior.lt.0) then
            write(*,3007) 'Symmetric',k1+mmax,maxm
          endif
        endif
      endif
      return
