...
c                   |   0      0   0 | 
          call pzero(f3d,3*3)
          do k = 1,nen
            f3d(1,1) = f3d(1,1) + ul(1,k)*shp(1,k)
            f3d(2,2) = f3d(2,2) + ul(2,k)*shp(2,k)
            f3d(1,2) = f3d(1,2) + ul(2,k)*shp(1,k) + ul(1,k)*shp(2,k)
          end do
          f3d(2,1) = f3d(1,2)
c
...
c....       compute residual r := - B^T*sigma*dv
            ...
            do i = 1,nen
              do ii = 1,2
                do jj = 1,3
                  p(i1+ii) = p(i1+ii) - bbar(jj,ii,i)*sig(jj)
                end do
	      end do
              i1 = i1 + ndf
            end do
...
c
      if (nel.eq.4) then
        do i = 1,nel
          shp(3,i) = 0.25d0*(1.d0 + eg(1)*rn(i))*(1.d0 + eg(2)*sn(i))
          sh0(1,i) = 0.25d0*rn(i)*(1.d0 + eg(2)*sn(i))
          sh0(2,i) = 0.25d0*sn(i)*(1.d0 + eg(1)*rn(i))
        end do
      endif
c
...
c
      do i = 1,2
        do j = 1,2
          xj(i,j) = 0.d0
          do ii = 1,nel
            xj(i,j) = xj(i,j) + xl(i,ii)*sh0(j,ii)
          end do
         end do
      end do
      ...
      do ii = 1,nel
        shp(1,ii) = xjinv(1,1)*sh0(1,ii)+ xjinv(2,1)*sh0(2,ii)
        shp(2,ii) = xjinv(1,2)*sh0(1,ii)+ xjinv(2,2)*sh0(2,ii)
      end do
      return
      end
c
